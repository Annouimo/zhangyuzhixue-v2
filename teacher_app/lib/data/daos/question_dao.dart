...
class QuestionDao {
  ...
  Future<db.QuestionRow?> getById(int id) async {
    ...
    AuditLogger.instance.dao('QuestionDao.getById', result != null ? 1 : 0, {
      'id': id,
    });
    return result;
  }
  ...
  Future<List<db.QuestionRow>> getByIds(List<int> ids) async {
    ...
    if (ids.length > 900) {
      ...
      for (var i = 0; i < ids.length; i += 500) {
        final chunk = ids.sublist(
          i,
          (i + 500 > ids.length) ? ids.length : i + 500,
        );
      ...
      AuditLogger.instance.dao('QuestionDao.getByIds', all.length, {
        'idsCount': ids.length,
      });
      return all;
    }
    ...
    AuditLogger.instance.dao('QuestionDao.getByIds', rows.length, {
      'idsCount': ids.length,
    });
    return rows;
  }
  ...
  Future<
    ({
      ...
      double diffMin,
      double diffMax,
      double gaokaoDiffMin,
      double gaokaoDiffAvg,
      double gaokaoDiffMax,
    })
  >
  searchStats({
    List<int>? years,
    ...
    List<String>? knowledgeCardNames,
    ...
    if (diffMin != null) {
      q.where((t) => t.difficulty.isBiggerOrEqual(Variable(diffMin)));
    }
    if (diffMax != null) {
      q.where((t) => t.difficulty.isSmallerOrEqual(Variable(diffMax)));
    }
    if (calcMin != null) {
      q.where((t) => t.calculation.isBiggerOrEqual(Variable(calcMin)));
    }
    if (calcMax != null) {
      q.where((t) => t.calculation.isSmallerOrEqual(Variable(calcMax)));
    }
    if (examTypes != null && examTypes.isNotEmpty) {
      q.where((t) => t.examType.isIn(examTypes));
    }
    if (questionTypes != null && questionTypes.isNotEmpty) {
      q.where((t) => t.questionType.isIn(questionTypes));
    }
    ...
    return (
      ...
      fill: fill,
      solution: solution,
      diffMin: minD == double.infinity ? 0.0 : minD,
      diffMax: maxD,
      gaokaoDiffMin: gkMin == double.infinity ? 0.0 : gkMin,
      gaokaoDiffAvg: gkAvg,
      gaokaoDiffMax: gkMax,
    );
  }
  ...
  Future<List<db.QuestionRow>> search({
    ...
    if (years != null) {
      q.where((t) => t.year.isIn(years.isEmpty ? const [-1] : years));
    }
    if (regions != null) {
      q.where((t) => t.region.isIn(regions.isEmpty ? const [''] : regions));
    }
    if (diffMin != null) {
      q.where((t) => t.difficulty.isBiggerOrEqual(Variable(diffMin)));
    }
    ...
    if (questionType != null) {
      q.where((t) => t.questionType.equals(questionType));
    }
    if (questionTypes != null) {
      q.where(
        (t) => t.questionType.isIn(
          questionTypes.isEmpty ? const [''] : questionTypes,
        ),
      );
    }
    if (examTypes != null) {
      q.where(
        (t) => t.examType.isIn(examTypes.isEmpty ? const [''] : examTypes),
      );
    }
    ...
    AuditLogger.instance.dao('QuestionDao.search', rows.length, {
      ...
      'knowledgeCards': knowledgeCardNames?.length,
    });
    return rows;
  }
  ...
  Future<List<int>> getDistinctYears() async {
    final rows = await _db
        .customSelect('SELECT DISTINCT year FROM question ORDER BY year')
        .get();
  ...
  Future<List<String>> getDistinctRegions() async {
    final rows = await _db
        .customSelect('SELECT DISTINCT region FROM question ORDER BY region')
        .get();
    final regions = rows.map((r) => r.read<String>('region')).toList();
    AuditLogger.instance.dao(
      'QuestionDao.getDistinctRegions',
      regions.length,
      {},
    );
    return regions;
  ...
  Future<List<String>> getDistinctExamTypes() async {
    ...
    AuditLogger.instance.dao(
      'QuestionDao.getDistinctExamTypes',
      types.length,
      {},
    );
    return types;
  ...
  Future<Map<int, db.ChoiceExtRow?>> getChoiceExtByQuestionIds(
    ...
    AuditLogger.instance.dao(
      'QuestionDao.getChoiceExtByQuestionIds',
      rows.length,
      {'ids': questionIds.length},
    );
    return map;
  }
  ...
  Future<List<db.SubQuestionRow>> getSubQuestions(int questionId) async {
    ...
    final rows = await q.get();
    AuditLogger.instance.dao('QuestionDao.getSubQuestions', rows.length, {
      'questionId': questionId,
    });
    return rows;
  }
  ...
  /// 批量查询子题
  Future<Map<int, List<db.SubQuestionRow>>> getSubQuestionsByQuestionIds(
    List<int> questionIds,
  ) async {
    if (questionIds.isEmpty) return {};
    final rows =
        await (_db.select(_db.subQuestions)
              ..where((t) => t.questionId.isIn(questionIds))
              ..orderBy([(t) => OrderingTerm(expression: t.sortOrder)]))
            .get();
    final map = <int, List<db.SubQuestionRow>>{};
    for (final r in rows) {
      map.putIfAbsent(r.questionId, () => []).add(r);
    }
    for (final id in questionIds) {
      map.putIfAbsent(id, () => []);
    }
    AuditLogger.instance.dao(
      'QuestionDao.getSubQuestionsByQuestionIds',
      rows.length,
      {'ids': questionIds.length},
    );
    return map;
  ...
  Future<List<db.SolutionMethodRow>> getMethods(int subQuestionId) async {
    ...
    q.orderBy([(t) => OrderingTerm(expression: t.sortOrder)]);
    final rows = await q.get();
    AuditLogger.instance.dao('QuestionDao.getMethods', rows.length, {
  ...
  Future<List<db.SolutionStepRow>> getSteps(int methodId) async {
    ...
    final rows = await q.get();
    AuditLogger.instance.dao('QuestionDao.getSteps', rows.length, {
      'methodId': methodId,
    });
    return rows;
  }
  ...
  Future<List<db.SolutionStepRow>> getStepsByMethodIds(
    List<int> methodIds,
  ) async {
    ...
    q.orderBy([(t) => OrderingTerm(expression: t.stepNumber)]);
    final rows = await q.get();
    AuditLogger.instance.dao('QuestionDao.getStepsByMethodIds', rows.length, {
      'methodIdsCount': methodIds.length,
    });
    return rows;
  }

  /// 批量查询多个子题的所有解法（替代 N+1 循环）
  Future<List<db.SolutionMethodRow>> getMethodsBySubQuestionIds(
    List<int> subQuestionIds,
  ) async {
    ...
    final rows = await q.get();
    AuditLogger.instance.dao(
      'QuestionDao.getMethodsBySubQuestionIds',
      rows.length,
      {'subQuestionIdsCount': subQuestionIds.length},
    );
    return rows;
  }
  ...
  Future<List<db.QuestionConceptTagRow>> getAllQuestionTagLinks() async {
    ...
    AuditLogger.instance.dao(
      'QuestionDao.getAllQuestionTagLinks',
      rows.length,
      {},
    );
    return rows;
  ...
  Future<List<db.ConceptTagRow>> getTagsByQuestion(int questionId) async {
    ...
    final rows = await q.get();
    AuditLogger.instance.dao('QuestionDao.getTagsByQuestion', rows.length, {
      'questionId': questionId,
    });
    return rows;
  }
  ...
  /// 批量查询题目标签
  Future<Map<int, List<db.ConceptTagRow>>> getTagsByQuestionIds(
    List<int> questionIds,
  ) async {
    if (questionIds.isEmpty) return {};
    final allLinks = await (_db.select(
      _db.questionConceptTags,
    )..where((t) => t.questionId.isIn(questionIds))).get();
    final tagIds = allLinks.map((l) => l.conceptTagId).toSet();
    final allTags = tagIds.isEmpty
        ? <db.ConceptTagRow>[]
        : await (_db.select(
            _db.conceptTags,
    ...
    AuditLogger.instance.dao(
      'QuestionDao.getTagsByQuestionIds',
      allLinks.length,
      {'ids': questionIds.length},
    );
    return result;
  ...
  Future<List<db.KnowledgeCardRow>> getKnowledgeCardsByQuestion(
    int questionId,
  ) async {
    final links = await (_db.select(
      _db.questionKnowledgeCards,
    ...
    final rows = await q.get();
    AuditLogger.instance.dao(
      'QuestionDao.getKnowledgeCardsByQuestion',
      rows.length,
      {'questionId': questionId},
    );
    return rows;
  ...
  /// 批量查询题目知识卡片
  Future<Map<int, List<db.KnowledgeCardRow>>> getKnowledgeCardsByQuestionIds(
    List<int> questionIds,
  ) async {
    if (questionIds.isEmpty) return {};
    final allLinks = await (_db.select(
    ...
    AuditLogger.instance.dao(
      'QuestionDao.getKnowledgeCardsByQuestionIds',
      allLinks.length,
      {'ids': questionIds.length},
    );
    return result;
  ...
  Future<List<db.KnowledgeCardRow>> getAllKnowledgeCards() async {
    ...
    AuditLogger.instance.dao(
      'QuestionDao.getAllKnowledgeCards',
      rows.length,
      {},
    );
    return rows;
  ...
  Future<db.KnowledgeCardRow?> getKnowledgeCardByTitle(String title) async {
    ...
    final result = await q.getSingleOrNull();
    AuditLogger.instance.dao(
      'QuestionDao.getKnowledgeCardByTitle',
      result != null ? 1 : 0,
      {'title': title},
    );
    return result;
