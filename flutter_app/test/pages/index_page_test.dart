  ...
  group('IndexPage', () {
    testWidgets('renders app bar with title', (tester) async {
      await tester.pumpWidget(
        const MaterialApp(home: IndexPage()),
      );
    ...
    testWidgets('renders quick entry grid', (tester) async {
      ...
      expect(find.text('📝 自主组卷'), findsOneWidget);
    ...
    testWidgets('tapping quick entry shows coming-soon snackbar', (tester) async {
      await tester.pumpWidget(
        const MaterialApp(home: IndexPage()),
      );

      await tester.tap(find.text('🤖 智能推荐'));
      await tester.pump();

      // SnackBar 显示后续版本实现
      expect(find.text('「🤖 智能推荐」将在后续版本中实现'), findsOneWidget);
    });
  });
}
