...
class _MockFavRepo implements ExamRepository {
  ...
  @override
  Future<List<FavoriteExamSummary>> getFavorites() async => list;
  @override
  ...
  group('ExamFavoritesPage', () {
    testWidgets('shows loading then list', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: ExamFavoritesPage(
            examRepository: _MockFavRepo(
              list: [
    ...
    testWidgets('shows empty state', (tester) async {
      await tester.pumpWidget(
        MaterialApp(home: ExamFavoritesPage(examRepository: _MockFavRepo())),
      );
      await tester.pumpAndSettle();
      expect(find.text('还没有收藏试卷，可以去发现页看看'), findsOneWidget);
    });
  });
}
