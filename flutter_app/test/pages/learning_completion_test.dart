...
void main() {
  ...
  for (final scenario in [
    ...
    testWidgets('${scenario.$1.name} completion offers a clear return action', (
      tester,
    ) async {
      var finished = false;
      await pumpUiScenario(
        tester,
        Scaffold(
          body: DoneBanner(onRate: () {}, onFinish: () => finished = true),
        ),
        viewport: scenario.$1,
        theme: scenario.$2,
      );
  ...
  testWidgets('quick practice uses the repeat action label', (tester) async {
    await pumpUiScenario(
      tester,
      Scaffold(
        body: DoneBanner(onNext: () {}, nextLabel: '再来一题'),
      ),
    );

    expect(find.text('再来一题'), findsOneWidget);
