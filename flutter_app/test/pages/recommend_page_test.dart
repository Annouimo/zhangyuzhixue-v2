...
class _MockRecommendRepo implements RecommendRepository {
  ...
  @override
  Future<List<RecommendPreset>> getPresets() async => List.generate(
    presetCount,
    (i) => RecommendPreset(id: i + 1, name: '预设 ${i + 1}'),
  );
...
void main() {
  ...
  group('RecommendPage', () {
    ...
    testWidgets('switches to preset when no smart data but presets exist', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
    ...
    testWidgets('manually switches between smart and preset via pills', (
      tester,
    ) async {
      final repo = _MockRecommendRepo(
        smartList: [
          const RecommendedQuestion(
            id: 1,
            title: '智能题',
      ...
      await tester.pumpWidget(
        MaterialApp(home: RecommendPage(recommendRepository: repo)),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('scrolls header and recommendation cards as one page', (
      tester,
    ) async {
      final questions = List.generate(
        6,
        (index) => RecommendedQuestion(
          id: index + 1,
          title: '推荐题 ${index + 1}',
          questionType: 'choice',
          difficulty: 3,
          recommendReason: '测试整页滚动',
          status: 'pending',
        ),
      );
      await tester.pumpWidget(
        MaterialApp(
          home: RecommendPage(
            recommendRepository: _MockRecommendRepo(smartList: questions),
          ),
        ),
      );
      await tester.pumpAndSettle();
      ...
      expect(find.text('把下一步练习交给推荐系统'), findsOneWidget);
      ...
      expect(find.text('把下一步练习交给推荐系统'), findsNothing);
      expect(find.text('推荐题 6'), findsOneWidget);
    });
  });
}
