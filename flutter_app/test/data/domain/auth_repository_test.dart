...
void main() {
  ...
  group('AuthRepository', () {
    test('login returns LoginResult', () async {
      adapter.on('POST', '/auth/login/', (_) => ResponseBody.fromString(
        '{"code":0,"data":{"access":"acc1","refresh":"ref1","user":{"id":1,"name":"stu","role":"student"}}}',
        200, headers: {'content-type': ['application/json']},
      ));
      final repo = AuthRepository(AuthApi(client));
      final r = await repo.login(LoginRequest(username: 's1', password: 'p', appType: 'student'));
    ...
    test('register succeeds', () async {
      adapter.on('POST', '/auth/register/', (_) => ResponseBody.fromString(
        '{"code":0,"data":null}', 200, headers: {'content-type': ['application/json']},
      ));
      final repo = AuthRepository(AuthApi(client));
      await repo.register(RegisterRequest(inviteCode: 'X', username: 'u', realName: 'n', phone: '138', gaokaoYear: '2026', password: 'p'));
    });

    test('refresh returns new token', () async {
      adapter.on('POST', '/auth/refresh/', (_) => ResponseBody.fromString(
        '{"code":0,"data":{"access":"new_acc"}}', 200, headers: {'content-type': ['application/json']},
      ));
      final repo = AuthRepository(AuthApi(client));
    ...
    test('logout succeeds', () async {
      adapter.on('POST', '/auth/logout/', (_) => ResponseBody.fromString(
        '{"code":0,"data":null}', 200, headers: {'content-type': ['application/json']},
      ));
      final repo = AuthRepository(AuthApi(client));
      await repo.logout();
