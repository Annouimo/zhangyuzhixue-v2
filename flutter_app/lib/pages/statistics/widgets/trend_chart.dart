...
class _TrendPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final chartRect = Rect.fromLTWH(_yAxisWidth, 0, size.width - _yAxisWidth, size.height - _xAxisHeight);
    ...
    final gridPaint = Paint()
      ...
      ..strokeWidth = 0.5;
    final yLabelStyle = TextStyle(fontSize: 9, color: AppColors.textMuted);
    for (var i = 0; i < 5; i++) {
      final y = chartRect.top + chartRect.height * i / 4;
      canvas.drawLine(Offset(chartRect.left, y), Offset(chartRect.right, y), gridPaint);
      // Y-axis label
      final tickVal = maxVal - (maxVal - minVal) * i / 4;
      final label = fixedYRange ? '${tickVal.toInt()}%' : tickVal.toStringAsFixed(0);
      final tp = TextPainter(text: TextSpan(text: label, style: yLabelStyle), textDirection: TextDirection.ltr)
        ..layout(maxWidth: _yAxisWidth - 4);
      tp.paint(canvas, Offset(_yAxisWidth - tp.width - 4, y - tp.height / 2));
    }

    // ── X-axis labels (first, middle, last) ──
    final xLabelStyle = TextStyle(fontSize: 9, color: AppColors.textMuted);
    for (final idx in [0, points.length ~/ 2, points.length - 1]) {
      final x = chartRect.left + idx * stepX;
      final label = points[idx].label.length >= 10 ? points[idx].label.substring(5, 10) : points[idx].label;
      final tp = TextPainter(text: TextSpan(text: label, style: xLabelStyle), textDirection: TextDirection.ltr)
        ..layout();
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      final y = chartRect.bottom - ((points[i].value - minVal) / valRange) * chartRect.height;
      pts.add(Offset(x, y));
