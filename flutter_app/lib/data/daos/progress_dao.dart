...
class QuestionReviewIds {
  final Set<int> currentWrong;
  final Set<int> corrected;

  const QuestionReviewIds({
...
class ProgressDao {
  ...
  Future<int> createSubmission({required int studentId}) async {
    final now = DateTime.now().toIso8601String();
    return _db
        .into(_db.submissions)
        .insert(
          db.SubmissionsCompanion(
  ...
  Future<List<db.SubmissionDetailRow>> getAttempts(int questionId) async {
    ...
    AuditLogger.instance.dao('ProgressDao.getAttempts', rows.length, {
  ...
  Future<db.SubmissionDetailRow?> getLatestAttempt(int questionId) async {
    final q = _db.select(_db.submissionDetails)
      ..where((t) => t.questionId.equals(questionId));
    q.orderBy([
      (t) => OrderingTerm(expression: t.attemptNumber, mode: OrderingMode.desc),
    ]);
    q.limit(1);
    final rows = await q.get();
    AuditLogger.instance.dao(
      'ProgressDao.getLatestAttempt',
      rows.isNotEmpty ? 1 : 0,
      {'questionId': questionId},
    );
  ...
  Future<int> createAttempt({
    ...
    final existing = await (_db.select(
      _db.submissionDetails,
    )..where((t) => t.questionId.equals(questionId))).get();
    final attemptNumber = existing.isEmpty
        ? 1
        : existing.map((e) => e.attemptNumber).reduce((a, b) => a > b ? a : b) +
              1;
    return _db
        .into(_db.submissionDetails)
        .insert(
          db.SubmissionDetailsCompanion(
  ...
  Future<void> updateAttemptAnswer(
    int id,
    String answerText,
    int isCorrect,
  ) async {
  ...
  Future<int> insertStepFeedback({
    ...
    final now = DateTime.now().toIso8601String();
    return _db
        .into(_db.stepFeedbacks)
        .insert(
          db.StepFeedbacksCompanion(
  ...
  Future<List<db.StepFeedbackRow>> getStepFeedbacks(
    ...
    final rows = await q.get();
    AuditLogger.instance.dao('ProgressDao.getStepFeedbacks', rows.length, {
  ...
  Future<int> insertCardFeedback({
    ...
    final now = DateTime.now().toIso8601String();
    return _db
        .into(_db.cardFeedbacks)
        .insert(
          db.CardFeedbacksCompanion(
            submissionDetailId: Value(submissionDetailId),
            questionId: Value(questionId),
  ...
  Future<List<db.CardFeedbackRow>> getCardFeedbacks(
    int submissionDetailId,
  ) async {
    final q = _db.select(_db.cardFeedbacks)
      ..where((t) => t.submissionDetailId.equals(submissionDetailId));
    final rows = await q.get();
    AuditLogger.instance.dao('ProgressDao.getCardFeedbacks', rows.length, {
      'submissionDetailId': submissionDetailId,
    });
    return rows;
  }
  ...
  Future<bool> hasAttempt(int questionId) async {
    ...
    AuditLogger.instance.dao('ProgressDao.hasAttempt', result.length, {
      'questionId': questionId,
    });
  ...
  Future<Set<int>> getAttemptedQuestionIds() async {
    ...
    AuditLogger.instance.dao(
      'ProgressDao.getAttemptedQuestionIds',
      ids.length,
      {},
    );
    return ids;
  ...
  Future<Set<int>> getCompletedQuestionIds() async {
    ...
    final ids = rows.map((r) => r.questionId).toSet();
    AuditLogger.instance.dao(
      'ProgressDao.getCompletedQuestionIds',
      ids.length,
      {},
    );
    return ids;
  ...
  Future<Set<int>> getRecentWrongQuestionIds(int days) async {
    ...
    AuditLogger.instance.dao(
      'ProgressDao.getRecentWrongQuestionIds',
      wrong.length,
      {'days': days},
    );
    return wrong;
  ...
  /// 以每题最新一次有判定结果的作答划分当前错题和已订正题。
  Future<QuestionReviewIds> getQuestionReviewIds() async {
    final query = _db.select(_db.submissionDetails)
      ..where((row) => row.isCorrect.isNotNull());
    final rows = await query.get();
    final byQuestion = <int, List<db.SubmissionDetailRow>>{};
    for (final row in rows) {
      byQuestion.putIfAbsent(row.questionId, () => []).add(row);
    }

    final currentWrong = <int>{};
    final corrected = <int>{};
    for (final entry in byQuestion.entries) {
      final attempts = entry.value
        ..sort((left, right) {
          final byAttempt = right.attemptNumber.compareTo(left.attemptNumber);
          return byAttempt != 0 ? byAttempt : right.id.compareTo(left.id);
        });
      final latest = attempts.first;
      if (latest.isCorrect == 0) {
        currentWrong.add(entry.key);
      } else if (attempts.skip(1).any((attempt) => attempt.isCorrect == 0)) {
        corrected.add(entry.key);
      }
    }
    AuditLogger.instance.dao(
      'ProgressDao.getQuestionReviewIds',
      currentWrong.length + corrected.length,
      {'currentWrong': currentWrong.length, 'corrected': corrected.length},
    );
    return QuestionReviewIds(currentWrong: currentWrong, corrected: corrected);
  }
  ...
  Future<int> getStuckStepCount(int questionId) async {
    ...
    AuditLogger.instance.dao('ProgressDao.getStuckStepCount', stuck, {
      'questionId': questionId,
    });
    return stuck;
  }
  ...
  Future<bool> hasRating(int questionId) async {
    ...
    AuditLogger.instance.dao('ProgressDao.hasRating', row != null ? 1 : 0, {
      'questionId': questionId,
    });
    return row != null;
  }
