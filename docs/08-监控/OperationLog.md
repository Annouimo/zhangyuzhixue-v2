...
1. 调用系统原生分享面板，选择目标 App（微信/QQ/文件管理器等）
2. 将日志文件（`operation_log.ndjson`）传递给所选 App
3. 用户直接在所选 App 中发送文件

**为什么从剪贴板改为系统分享面板**：
- 第三方剪贴板管理软件（Ditto、ClipX 等）会截断长文本，日志超出其限制时丢失数据
- 文件分享无大小限制，始终可靠
- 使用 `share_plus` 包，跨平台（Android/iOS/Windows/macOS）调用原生分享面板
- 移动端可直达微信/QQ，桌面端可保存到文件夹后手动发送
